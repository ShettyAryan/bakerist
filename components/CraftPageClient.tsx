    ...
    <main className="bg-cream dark:bg-bg-dark transition-colors duration-500">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-6 pt-40 pb-24">
          <div className="reveal">
            <p className="text-[10px] font-bold text-white/55 tracking-[0.35em] uppercase mb-6 font-brand">
              Our Craft
            </p>
            <h1 className="font-display text-white text-[clamp(2.8rem,6vw,4.6rem)] leading-[1.02] mb-5">
              {craft.title}
            </h1>
            <p className="text-white/75 text-sm md:text-[1.05rem] font-sans font-light tracking-wide mb-8 max-w-2xl">
              {craft.sub}
            </p>
            <p className="text-white/70 leading-[1.9] font-sans font-light max-w-2xl">
              {craft.description}
            </p>
          </div>

          <div
            className="mt-12 flex flex-col sm:flex-row gap-4 reveal"
            style={{ transitionDelay: "120ms" }}
          >
            <Link
              href="/"
              className="inline-flex justify-center items-center border border-white/35 text-white
                         px-10 py-4 rounded-full uppercase tracking-[0.22em] text-[10px] font-bold font-brand
                         hover:bg-white hover:text-stone-900 transform hover:-translate-y-1
            ...
            <Link
              ...
              className="inline-flex justify-center items-center btn-shimmer text-white
                         px-10 py-4 rounded-full uppercase tracking-[0.22em] text-[10px] font-bold font-brand
                         shadow-lg hover:shadow-primary/25 transform hover:-translate-y-1
                         transition-all duration-300"
            >
              Find a Location
            </Link>
      ...
      <section className="py-20">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-0 border border-stone-200/70 dark:border-stone-700/60 rounded-3xl overflow-hidden bg-white/60 dark:bg-surface-dark/70 backdrop-blur">
            {craft.highlights.map(({ label, detail }, i) => (
              <div
                key={label}
                className={`p-10 text-center reveal ${
                  i < craft.highlights.length - 1
                    ? "md:border-r border-stone-200/70 dark:border-stone-700/60"
                    : ""
                }`}
                style={{ transitionDelay: `${i * 90}ms` }}
              >
                <p className="font-display text-primary text-3xl font-bold mb-2">
                  {label}
                </p>
      ...
      <section className="pb-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-end justify-between gap-8 mb-12 reveal">
            <div>
              <p className="text-[10px] font-bold text-primary tracking-[0.35em] uppercase mb-4 font-brand">
                Curated Menu
              </p>
              <h2 className="font-display text-stone-900 dark:text-stone-50 text-[clamp(2rem,4vw,3.1rem)]">
                Signature Selection
              </h2>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {craft.products.map((p, i) => (
              <div
                key={p.name}
                className="group relative rounded-3xl border border-stone-200/70 dark:border-stone-700/60
                           bg-white/70 dark:bg-surface-dark/70 backdrop-blur overflow-hidden
                           shadow-lg hover:shadow-xl transition-all duration-500 reveal"
              ...
              >
                ...
                <div className="relative aspect-[4/3] overflow-hidden">
                  <Image
                    ...
                    className="object-cover transition-transform duration-700 group-hover:scale-105 ease-expo"
                  />
                ...
                <div className="relative p-8">
                  <div className="flex items-start justify-between gap-6">
                    <div>
                      <h3 className="font-display text-stone-900 dark:text-stone-50 text-2xl leading-tight mb-2">
                        {p.name}
                      </h3>
                      <p className="text-stone-500 dark:text-stone-400 text-sm leading-[1.8] font-sans font-light">
                        {p.desc}
                      </p>
                    </div>
                    <div className="shrink-0 text-right">
                      ...
                      <div className="mt-3 inline-flex items-center gap-1.5 text-stone-400 dark:text-stone-500">
                        <ArrowUpRight
          ...
          <div className="mt-14 text-center reveal" style={{ transitionDelay: "80ms" }}>
            <Link
              href="/#menu"
              className="inline-flex justify-center items-center border border-stone-900/70 dark:border-stone-400/40
                         text-stone-900 dark:text-stone-200 px-14 py-5 rounded-full uppercase tracking-[0.22em]
                         text-[10px] font-bold font-brand hover:bg-stone-900 hover:text-white
                         dark:hover:bg-stone-100 dark:hover:text-stone-900 transform hover:-translate-y-1
                         shadow-lg hover:shadow-xl transition-all duration-400"
            >
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex items-end justify-between gap-8 mb-12 reveal">
            <div>
              ...
              <h2 className="font-display text-stone-900 dark:text-stone-50 text-[clamp(2rem,4vw,3rem)]">
                More from Our Craft
              </h2>
            </div>
            <Link
            ...
            {otherCrafts.map((c, i) => (
              ...
              >
                ...
                <div className="absolute inset-0 flex flex-col justify-end p-8">
                  ...
                  <h3 className="font-display text-white text-3xl mb-2 leading-tight">
                    {c.title}
                  </h3>
                  <p className="text-stone-300 text-sm font-sans font-light leading-snug max-w-[28ch]">
                    {c.sub}
                  </p>
                  <div className="mt-6 inline-flex">
                    <span
                      className="bg-white/10 border border-white/15 text-white px-6 py-2.5 rounded-full
                                 text-[9px] font-bold uppercase tracking-[0.2em] transition-colors font-brand
                                 group-hover:bg-primary/90 group-hover:border-primary/50"
                    >
                      View Selection
                    </span>
                  </div>
